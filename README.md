
# Hi, I'm Rayane Krachni

**Software Engineer | Full Stack Developer**
 Focused on **Backend Development & System Design**

---

###  About Me

*  Currently working with **Next.js** for web and **Flutter** for mobile applications
*  Building **full stack applications** 
*  Actively improving my skills in **backend engineering** and deepening my understanding of **system design**
*  Passionate about creating **efficient, scalable, and production-ready systems**

---

###  What I Work With

*  **Frontend:** Next.js, React
*  **Mobile:** Flutter (iOS & Android)
 *  **Backend:** Django
*  **System Design:** scalability, architecture patterns

---

###  Current Focus

* Strengthening **backend architecture skills**
* Mastering **system design fundamentals** (scalability, reliability, performance)
* Building **end-to-end full stack & mobile applications**
* Exploring **microservices & distributed systems**

---

### Let's Connect

📄 More details on my LinkedIn: <a target="_blank" href="https://www.linkedin.com/in/rayane-krachni-b1794619a" style="display: inline-block;"> <img src="https://img.shields.io/badge/linkedin-logo?style=for-the-badge&logo=linkedin&logoColor=white&color=%230a77b6" alt="linkedin" /> </a>

